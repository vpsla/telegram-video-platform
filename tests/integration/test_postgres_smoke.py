...
async def test_ilike_search_is_case_insensitive_on_postgres(
    ...
    """Pins down ILIKE case-insensitivity against the real production
    dialect, rather than trusting SQLite's (different) LIKE semantics
    by inference."""
...
async def test_bigint_telegram_id_roundtrip_on_postgres(
    ...
    """Telegram user IDs can legitimately exceed 32-bit signed int range;
    this pins down that our BigInteger column actually stores values
    beyond int32 without truncation or overflow error on real Postgres."""
